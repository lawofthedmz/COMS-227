...
public class Lizard {
	...
	public BodySegment getSegmentAt(Cell cell) {
		for (BodySegment segment : segments) { // Iterates through BodySegment 
			if (segment.getCell().equals(cell)) { // Compares Current Segment and return segment if it matches cell 
	            return segment; 
			}
		}
		return null; // Segment not found at the given cell
	}
	...
	public Direction getHeadDirection() {
        ...
        if (segmentBehindHead == null) {
            return null; // This should not happen if the segments are properly ordered
        }
    ...
    private Direction directionFromTo(Cell from, Cell to) {
        ...
        if (dx == 0 && dy == -1) {
            ...
            return null; // Not directly adjacent
        }
    }
