...
//  }
  ...
  public void addCoins(int howMany)
  {
    // if the piggy bank is already full or smashed, do nothing
    if (isFull() || isSmashed()) {
        return;
    }
    ...
    // to add all the coins
    int amountOfRoom = maxCoins - coins;
    int newCoins = Math.min(howMany, amountOfRoom); // use Math.min to avoid exceeding maxCoins
    
    coins += newCoins; // increment the current number of coins
  }
...
//  }
  public boolean isSmashed()
  {
    return busted; // simply return the value of busted
  }
